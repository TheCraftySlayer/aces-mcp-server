...
# Optional admin token for viewing /task-cache.
...
async def task_cache_debug(request):
    ...
    if supplied_token != ACES_ADMIN_TOKEN:
        return JSONResponse(
...
async def _call_customgpt_task(
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if final.status_code >= 400:
            error_text = _safe_json_dumps(final_data)
        ...
        if not answer:
            _remember_task(
                ...
                message_id=str(message_id),
                progress_log=progress_log,
                error=_safe_json_dumps(final_data),
            )
